# Публикация NPM
Я сделал эту библиотек для собственных нужд, потому что меня задолбало копирование файлов из проекта в проект. Никогда ранее мне не приходилось публиковать npm-пакеты, поэтому пришлось гуглить как это делается.
...
### 8.1
Я решил использовать в названии пакета имя своего аккаунта т.е. в терминологии npm у меня `scoped package`. По умолчанию такие пакеты считаются приватными и для их публикации в первый раз надо использовать специальный флаг:
```
npm publish --access public
```
### 8.2
Прежде чем реально публиковать пакет, неплохо было бы его протестировать. Для этого есть специальные команды `npm link` и `npm unlink`. Но облом состоит в том, что даже если вы перенесете зависимости в правильные секции, при линковке будет происходить ерунда. По крайней мере у меня именно так. Есть различные рецепты решения этой проблемы, но в основном они упираются в использование чего-то типа `rollup` в сборке пакета, а я не хочу наворачивать еще лишних компонентов в проект. Поэтому просто забил.
